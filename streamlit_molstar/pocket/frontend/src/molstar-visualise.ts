...
export async function loadStructureIntoMolstar(plugin: PluginUIContext, structureUrl: string, format: string) {
    const data = await plugin.builders.data.download({
        url: Asset.Url(structureUrl),
        isBinary: false
    }, {state: {isGhost: true}});
    return loadStructureIntoMolstarFromData(plugin, data, format);
}
/**
 ...
 */
export async function loadStructureIntoMolstarFromData(plugin: PluginUIContext, data: any, format: string) {
    let trajectory;

    if(format.endsWith("cif")) trajectory = await plugin.builders.structure.parseTrajectory(data, "mmcif");
